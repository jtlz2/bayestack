...
#dataset='video'
binStyle=1
nlaws=1 # Number of ppl laws (or poly coefficients) to fit
floatNoise=True
modelFamily='ppl'#'ppl' 'poly' # 'LFsch' # 'LFdpl' # 'HIsch' # 'HIdpl'
outdir='chains_160505a' # based on 151116x
noiseZones=[0] # Select which noise zones to analyse
doPoln=True
doRayleigh=False # Set Rice signal to zero, i.e. Rayleigh distribution - *** use nlaws = 1
shiftedPosns=False # Use offset catalogue
randomPosns=False # Use random-posn catalogue
u1x=True # Use original (x1) extractions
uMask=False # Use masked extractions
maskSERVS=False # Use SERVS mask rather than any other
maskSERVSplusPOLN=False # Use SERVSplusPOLN mask rather than any other
doRedshiftSlices=False # For joint z slice analysis
...
comment='JVLA 5GHz - pre-final runs'
...
#-------------------------------------------------------------------------------
shiftedIntensityPosns=shiftedPolnPosns=shiftedPosns
randomIntensityPosns=randomPolnPosns=randomPosns
# Extraction settings
perGalMode=False
...
#    perGalGalaxyType='sbn'
    ...
    injectedMapDir=os.path.join('/Users/jtlz2/elaisn1/bayestack-archive/sims-archive',dataset)
    ...
    if useInjectedMap:
        ...
        if doPoln:
            perGalRadioMap='/Users/jtlz2/Dropbox/elaisn1/maps/EN1.Pave.mosaic_vla.fits'
        else:
    ...
    if perGalUpsampleRate is not None and perGalUpsampleRate < 0:
        if useInjectedMap:
            ...
            if doPoln:
                upsampledMap='/Users/jtlz2/elaisn1/maps/EN1.Pave.mosaic_vla_x%i.fits'%abs(perGalUpsampleRate)
            else:
                upsampledMap='/Users/jtlz2/elaisn1/maps/EN1_WITHBEAM_vla_x%i.fits'%abs(perGalUpsampleRate)
    else:
        upsampledMap=None
    if perGalUpsampleRate is not None:
        upsampledNoise='/Users/jtlz2/elaisn1/maps/EN1.I.mosaic.sensitivity_vla_x%i.fits'% abs(perGalUpsampleRate)
    else:
...
if dataset=='cosmos':
    ...
    cutsDict={'star':[30,0],'lacy':[34,-1],'stern':[38,-1],'donley':[40,-1],\
              'noise0':[47,0.8,1.1],'noise1':[47,1.1,1.5],\
              'noise2':[47,1.5,2.0],\
              'noise3':[47,2.0,2.5],'noise4':[47,2.5,3.0],\
              'noise5':[47,3.0,5.0],'noise6':[47,5.0,13.0]}
              #'noise0':[47,1.5,2.0]}
              #'noise0':[47,1.1,1.5]}
    ...
    numRedshiftSlices=len(whichRedshiftSlices)
elif dataset=='goodsnx':
    if not doPoln:
        stokes='I'
    else:
        stokes='P'
    weightvlaf='/Users/jtlz2/goodsn/maps/GNPilot.centre.flux.%s.fits' % stokes
    noisezonesf='noisezones_%s.txt'%stokes
    noisezonesf=os.path.join(dataset,noisezonesf)
    cutsDict={'noise0':[8,0.7,1.0],'noise1':[8,1.0,1.5],\
              #'noise0':[7,1.5,2.0]}
              #'noise0':[7,1.1,1.5]}
              'noise2':[8,1.5,2.0],\
              'noise3':[8,2.0,2.5]}
    numNoiseZones=len([k for k in cutsDict.keys() if 'noise' in k])
    datafiles=['goodsnx_%s_a%i.txt'%(stokes,n) for n in noiseZones]
    datafile=datafiles[0]
    SURVEY_AREAS={}; SURVEY_NOISES={}
    if not os.path.exists(noisezonesf):
        nz=numpy.zeros((numNoiseZones,4))
    ...
    SURVEY_AREA=sum([a for (f,a) in SURVEY_AREAS.iteritems()])
    redshifts=[0.0 for i in range(numNoiseZones)]
    SURVEY_AREA=4.433808e-03
elif dataset=='edcube':
    SURVEY_NOISE=100.0 # uJy
    SURVEY_AREA=1.0 # sq. deg.
    datafiles={}
    datafiles[0]=datafile='edcube/total_fluxes.txt'
    #numRedshiftSlices=zmanifest.shape[0]
    #redshifts=zmanifest[[s-1 for s in whichRedshiftSlices],3]
    numRedshiftSlices=len(whichRedshiftSlices)
...
if context=='b' or True:
    ...
    elif 'edcube' in dataset:
        BIN_CAT_FORM=10
        BIN_CAT='edcube/total_fluxes.txt'
        BIN_CAT_CLIP=None
        BIN_COL=2
        BOUT_CAT='edcube/edcube.txt'

    elif dataset == 'en1jvla':
        BIN_CAT_FORM=8 # See /Users/jtlz2/elaisn1/servs/scripts/*.stil
        if shiftedIntensityPosns:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted800p800p-151116.txt'
        elif randomIntensityPosns:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-151116.txt' # Check this file is present
        else:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-151116.txt'
#        if doPoln:
        #shiftedPolnPosns=False
        #randomPolnPosns=False

        if shiftedPolnPosns:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted800p800p-w-poln-151116.txt'
        elif randomPolnPosns:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-151116.txt'
        else:
            BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-w-poln-with-detns-151116.txt'

        if u1x:
            if uMask:
                maskf='/Users/jtlz2/Dropbox/elaisn1/maps/mask.fits'
                if shiftedPolnPosns:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted100p100p-w-poln-masked-160310.txt'
                elif randomPolnPosns:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-masked-160310.txt'
                    if maskSERVS:
                        BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-servsmasked-160420.txt'
                        maskf='/Users/jtlz2/Dropbox/elaisn1/maps/mask_empty.fits'
                    elif maskSERVSplusPOLN:
                        BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-servspolnmasked-160503.txt'
                        maskf='/Users/jtlz2/Dropbox/elaisn1/maps/mask_empty_poln.fits'
                else:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-w-poln-with-detns-masked-160310.txt'
            else:
                if shiftedPolnPosns:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted100p100p-w-poln-160223.txt'
                elif randomPolnPosns:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-160223.txt'
                else:
                    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-w-poln-with-detns-160223.txt'
                maskf=None
        ...
        BIN_CAT_FORM=9 # See /Users/jtlz2/elaisn1/servs/scripts/*.stil
        #BIN_CAT='/Users/jtlz2/Dropbox/bayestack/goodsnx/goodsnx_151214.txt'
        shiftedPolnPosns=False
        randomPolnPosns=False
        if shiftedPolnPosns:
            BIN_CAT='/Users/jtlz2/Dropbox/bayestack/goodsnx/goodsnx_160125_shift800p800p.txt'
        elif randomPolnPosns:
            BIN_CAT='/Users/jtlz2/Dropbox/bayestack/goodsnx/goodsnx_160125_random.txt'
        else:
            BIN_CAT='/Users/jtlz2/Dropbox/bayestack/goodsnx/goodsnx_160125.txt'
        #BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted800p800p-151116.txt'
        #BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-1511116.txt'
#        if doPoln:
        #    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-shifted800p800p-w-poln-151116.txt'
        #elif randomPolnPosns:
        #    BIN_CAT='/Users/jtlz2/elaisn1/servs/servs-en1-full-data-fusion-sextractor-cutdown-masked-vla-overlap-with-detns-random-w-poln-151116.txt'
        ...
        BIN_CAT_CLIP=None
        if doPoln:
            BIN_COL=10 # peak P flux
        else:
            BIN_COL=7 # peak I flux
        BOUT_CAT='goodsnx/goodsnx_%s.txt'%stokes
#-------------------------------------------------------------------------------
...
if context=='u' or context=='i':
    ...
    if upsamplingFactor is not None:
        ...
        if doPoln:
            outputMap='/Users/jtlz2/elaisn1/maps/EN1.Pave.mosaic_vla_x%i.fits' % upsamplingFactor
        else:
            outputMap='/Users/jtlz2/elaisn1/maps/EN1_WITHBEAM_vla_x%i.fits' % upsamplingFactor
        outputNoiseMap='/Users/jtlz2/elaisn1/maps/EN1.I.mosaic.sensitivity_vla_x%i.fits' % upsamplingFactor
    if doPoln:
        injectInputMap='/Users/jtlz2/Dropbox/elaisn1/maps/EN1.Pave.mosaic_vla.fits'
    else:
        injectInputMap='/Users/jtlz2/Dropbox/elaisn1/maps/EN1_WITHBEAM_vla.FITS'
    injectInputNoiseMap='/Users/jtlz2/Dropbox/elaisn1/maps/EN1.I.mosaic.sensitivity_vla.fits'
    #if useInjectedMap and context != 'i':
    if useInjectedMap and context != 'i':
        inputMap=os.path.join(dataset,injectedMap)
        outputMap=os.path.join(dataset,'sim_map_x2.fits')
    else:
        inputMap=injectInputMap
        inputNoiseMap=injectInputNoiseMap
...
SMAX_MIN=1.5
#SMAX_MAX=5.8
SMAX_MAX=binsZoned[noiseZones[0]][-1]
...
labelDict={'C':r'$C/$Jy$^{-1}$sr$^{-1}$',\
           ...
           'delta':r'$\delta$','S2':r'$S_2/\mu$Jy','S3':r'$S_3/\mu$Jy',\
           'S4':r'$S_4/\mu$Jy','sigma':r'$\sigma/\mu$Jy',\
           ...
           'noise':r'$\sigma/\mu$Jy',\
           'a0':r'$\alpha_0$','a1':r'$\alpha_1$','a2':r'$\alpha_2$','a3':r'$\alpha_3$'}
