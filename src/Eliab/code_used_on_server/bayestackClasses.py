...
class dataSetup(object):
    ...
    def __init__(self,whichSurvey,manifestf,noiseZones=False,redshiftSlices=False):
        ...
        if self.redshiftSlices:
            self.datafiles=['%s/data_cos_s%i.txt'%('cos_data',r) for r in self.whichRedshiftSlices]
...
class countModel(object):
    ...
    def parsePriors(self,parameters,floatNoise):
        ...
        for p in parameters:
            ...
            elif p=='LMIN_1': priorsDict['LMIN_1']=[LMIN_1_PRIOR,LMIN_MIN_1,LMIN_MAX_1]
            elif p=='LMIN_2': priorsDict['LMIN_2']=[LMIN_2_PRIOR,LMIN_MIN_2,LMIN_MAX_2]
            elif p=='LMIN_3': priorsDict['LMIN_3']=[LMIN_3_PRIOR,LMIN_MIN_3,LMIN_MAX_3]
            elif p=='LMIN_4': priorsDict['LMIN_4']=[LMIN_4_PRIOR,LMIN_MIN_4,LMIN_MAX_4]
            elif p=='LMIN_5': priorsDict['LMIN_5']=[LMIN_5_PRIOR,LMIN_MIN_5,LMIN_MAX_5]
            elif p=='LMIN_6': priorsDict['LMIN_6']=[LMIN_6_PRIOR,LMIN_MIN_6,LMIN_MAX_6]
            elif p=='LMIN_7': priorsDict['LMIN_7']=[LMIN_7_PRIOR,LMIN_MIN_7,LMIN_MAX_7]
            elif p=='LMIN_8': priorsDict['LMIN_8']=[LMIN_8_PRIOR,LMIN_MIN_8,LMIN_MAX_8]
            elif p=='LMIN_9': priorsDict['LMIN_9']=[LMIN_9_PRIOR,LMIN_MIN_9,LMIN_MAX_9]
            elif p=='LMIN_10': priorsDict['LMIN_10']=[LMIN_10_PRIOR,LMIN_MIN_10,LMIN_MAX_10]
            ...
            elif p=='B_agn':priorsDict['B_agn']=[B_agn_PRIOR,B_agn_MIN,B_agn_MAX]
            elif p=='B_SF':priorsDict['B_SF']=[B_SF_PRIOR,B_SF_MIN,B_SF_MAX]
            elif p=='A_agn':priorsDict['A_agn']=[A_agn_PRIOR,A_agn_MIN,A_agn_MAX]
	    elif p=='Nsigma_a':priorsDict['Nsigma_a']=[Nsigma_a_PRIOR,Nsigma_a_MIN,Nsigma_a_MAX]
	    
  	    elif p=='A_D':priorsDict['A_D']=[A_D_PRIOR,A_D_MIN,A_D_MAX]
            elif p=='B_D':priorsDict['B_D']=[B_D_PRIOR,B_D_MIN,B_D_MAX]
...
#                        1.0) for S in self.binsMedian/1.0e6]
    	...
    	elif self.kind in ['LFsch','LFpl','LFdpl','LFlognorm','LFdpl_pl','LFlognorm_dpl','LFdpl_dpl']:
    ...
    def evaluate2(self,params):
        ...
        for r in range(n):
            ...
            Vmax = self.zDataObject.Vmax[z]
                #print z,n,r
            evaluations.append([lumfuncUtils.dNdS_LF(S,z,dlds,Vmax,dl,params,\
                self.parameters,inta= None,\
    ...
    def realise(self,cube):
        if self.doPoln:
        ...
        elif self.kind in ['LFsch','LFdpl','LFpl','LFdpl_pl','LFpl_dpl','LFpl_dpl',\
			   'LFdpl_dpl','LFlognorm','LFlognorm_dpl','LFpl_lognorm',\
			   'LFdpl_dpl_z','LFevol_dpl','LFevol_logn','LFevol_dpl_s',\
			   'LFevol_logn_s','LFevol_dpl_L','LFevol_logn_L',\
			   'LFevol_logn_mat','LFevol_logn_el','LFevol_logn_slope',\
			   'LFevol_logn_sigma','LFevol_logn_lnorm','LFevol_logn_lmin',\
			   'LFevol_phi_logn_mat','LFevol_phi_logn','LFevol_logn_all',\
			   'LFevol_logn_all_L']:
            self.zRealiseObject={}
            for r in range(len(sorted(self.zDataObject.zsliceData.keys()))):
                ...
                #print 'start II'
		if not floatNoise:
		    NOISE=self.zDataObject.sliceNoises[z]
                else:NOISE=0
                self.zRealiseObject[z]=lumfuncUtils.calculateL3(cube,self.parameters,\
                    ...
                    redshift=z,dl=dl,dlds=dlds,Vmax=Vmax,mag=mag,noise=NOISE)
            ...
            if self.kind in [ 'LFlognorm_dpl','LFdpl_pl','LFdpl_dpl']:
               ...
               if not (cube[self.parameters.index('LMIN')]< cube[self.parameters.index('LSTAR_2')]<\
                    ...
                    return poissonLhoodMulti2(self.zDataObject,self.realise(cube),\
                                         silent=True)
	    elif self.kind in [ 'LFpl_dpl','LFdpl_dpl_z']:
               if not (cube[self.parameters.index('LMIN')]< cube[self.parameters.index('LSTAR')]<\
                        cube[self.parameters.index('LMAX2')] and cube[self.parameters.index('LMIN2')]<\
                        cube[self.parameters.index('LSTAR_2')]<cube[self.parameters.index('LMAX')]):

                    return -1.0e99
               else:
                    return poissonLhoodMulti2(self.zDataObject,self.realise(cube),\
                                         silent=True)
	    elif self.kind in [ 'LFpl_lognorm']:
               if not (cube[self.parameters.index('LSTAR_2')]<\
                        cube[self.parameters.index('LMAX2')] and cube[self.parameters.index('LMIN2')]<\
                        cube[self.parameters.index('LSTAR')]<cube[self.parameters.index('LMAX')]):
                ...
                if not (cube[self.parameters.index('LMIN')]< cube[self.parameters.index('LSTAR_2')]<\
                    ...
                    #sys.exit()					
                    return pois
            elif  self.kind in ['LFevol_dpl','LFevol_logn','LFevol_dpl_s','LFevol_logn_s','LFevol_dpl_L','LFevol_logn_L','LFevol_logn_mat','LFevol_logn_slope','LFevol_logn_sigma','LFevol_logn_lnorm','LFevol_logn_all','LFevol_logn_lmin','LFevol_logn_el','LFevol_phi_logn_mat','LFevol_phi_logn','LFevol_logn_all_L']:
                return poissonLhoodMulti2(self.zDataObject,self.realise(cube),\
                                         silent=True)
            else:
