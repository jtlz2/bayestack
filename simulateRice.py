...
from utils import *
from countUtils import powerLawFuncS
...
# This plot is f(Q0,U0) [U, DELTA, G, etc.] + n -> f(P0) + n -> g(P)|f(P0), showing:
...
plt.hist(P,bins=bins,color='b',alpha=0.5,label=r'P$\equiv\sqrt{Q^2+U^2}$') # noisy data
plt.hist(Pprime,bins=bins,color='r',alpha=0.5,         label=r'Rice(%3.1f,%3.1f)'%(numpy.sqrt(Q0**2+U0**2),sigma_QU))
...
print area
area2 = integrate.dblquad(lambda p,p0:1.0*F(p,p0,1.0*sigma_QU),1.0,20.0,                          lambda p:2.0,lambda p:2.5)
print area2
