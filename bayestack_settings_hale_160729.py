...
floatNoise=True
modelFamily='ppl'#'ppl' 'poly' # 'LFsch' # 'LFdpl'
noiseZones=[0] # Select which noise zones to analyse
outdir='chains_160729_n%i'%(noiseZones[0]) # based on 140123a
doPoln=False
...
nb= 27#40#38#40#39#37#41#50#39#37 #13 #24 #27 #34 #37  # 38 or 41
...
#    numNoiseZones=len([k for k in cutsDict.keys() if 'noise' in k])
    ...
    #datafile='/users/hale/Data/bootes/bootes-out-cat-radius--0.8.txt'  # uJy
    datafile='/users/hale/Data/bootes/Bootes-optical-catalogue-with-fluxes_ra_dec-no-nan-mixJy-noaocut-10arcsec-with-LOFAR-sources-excluded-flux-binning.txt'
    #SURVEY_AREA=0.0789132395988 # sq. deg.
    #SURVEY_NOISE=108.918742143 # uJy
    SURVEY_NOISE=115.0 #uJy  best value in the centre
    if not doPoln:
        stokes='I'
    else:
        stokes='P'
    #weightvlaf='/users/hale/Data/bootes/rms-mosaic-uJy-radius-0.8.fits'
    #weightvlaf='/users/hale/Data/bootes/rms-mosaic-uJy-radius-0.8.fits'
    #weightvlaf='/users/hale/Data/bootes/rms-mosaic-uJy-radius-0.8.fits'
    weightvlaf='/users/hale/Data/bootes/rms-mosaic-mixJy-v4-noaocut-exclude-region-around-lofar-flux-binning.fits'
    noisezonesf='noisezones_%s_%s.txt'%(stokes, run_num)
    noisezonesf=os.path.join(dataset,noisezonesf)
#    cutsDict={'star':[30,0],'lacy':[34,-1],'stern':[38,-1],'donley':[40,-1],\
...
#              'noise4':[47,3.0,5.0],'noise5':[47,5.0,13.0]}
    #cutsDict={'noise0':[1,90.0,150.0]}   # chains_190516a
    #cutsDict={'noise0':[1,90.0,100.0], 'noise1':[1,100.0,110.0], 'noise2':[1,110.0,120.0], 'noise3':[1,120.0,130.0], 'noise4':[1,130.0,140.0], 'noise5':[1,140.0,150.0]}
    cutsDict={'noise0':[1,110.0,120.0], 'noise1':[1,120.0,130.0], 'noise2':[1,130.0,140.0], 'noise3':[1,140.0,150.0]}
#    numNoiseZones=len([k for k in cutsDict.keys() if 'noise' in k])
    datafiles=['bootes-test-cat_%s_a%i.txt'%(run_num,n) for n in noiseZones]
    datafile=datafiles[0]
    SURVEY_AREAS={}; SURVEY_NOISES={}
    if not os.path.exists(noisezonesf):
        nz=numpy.zeros((numNoiseZones,4))
    ...
    if not doPoln:
        ...
        SURVEY_NOISE=SURVEY_NOISE
    #SURVEY_AREAS[0]=SURVEY_AREA
    #SURVEY_NOISES[0]=SURVEY_NOISE
    SURVEY_AREA=sum([a for (f,a) in SURVEY_AREAS.iteritems()])
...
if context=='b' or True:
    ...
    if dataset == 'first':
        ...
        CORR_BINS=numpy.ones(nb)
        BIN_COL=0 # - pixel flux
        BIN_CAT=os.path.join(dataset,'/users/hale/Data/bootes/Bootes-optical-catalogue-with-fluxes_ra_dec-no-nan-mixJy-noaocut-10arcsec-with-LOFAR-sources-excluded-flux-binning.txt')
        CORR_RESOLUTION=None
        BOUT_CAT=os.path.join(dataset,'bootes-test-cat_%s.txt') %(run_num)
...
elif binstyle=='bootes':
    #bins=numpy.linspace(-270.0,1000.0,8)
    #bins=numpy.array([-80.0,-50.0,-20.0,-10.0,-5.0,0.0,5.0,10.0,20.0,\
    #                    50.0,100.0,200.0,300.0,400.0,500.0,600.0,700.0,800.0])
    #bins=numpy.linspace(0.0,130000.0,100001)
    #bins=numpy.array([-560,-320,-180,-100,-56,-32,-18,-10,-6,-3,-2,-1,1,2,3,6,10,18,32,56,100,180,320,560,1000,1800,3100,5600,10000,32000])
    bins=numpy.array([-540,-320,-180,-100,-56,-32,-18,-10,-6,-3,-2,-1,1,2,3,6,10,18,32,56,100,180,320,560,1000,1800,3100,5000])
    #bins=numpy.linspace(-20000.0,100000.0,101)
    #bins=numpy.array([-80.0,-65.0,-50.0,-40.0,-30.0,-20.0,-15.0,-10.0,-8.0,-6.5,-5.0,-3.5,-2.5,-1.0,-0.65,-0.5,-0.25,-0.1,-0.05,0.0,0.05,0.1,0.25,0.5,0.65,1.0,2.5,3.5,5.0,6.5,8.0,10.0,15.0,20.0,30.0,40.0,50.0,65.0,85.0])
    #bins[0]=-69.0
...
SLOPE_MAX=0.0
SMIN_MIN=0.01
SMIN_MAX=SURVEY_NOISE
SMAX_MIN=SURVEY_NOISE
SMAX_MAX=5000.
