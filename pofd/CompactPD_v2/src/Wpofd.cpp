...
double output_res( int Nbins, double * xbin_min, double * xbin_max, double * data, double Delta, int N, double * out, double sigma_noise, double * result){
...
printf("Ncut %d\n",Ncut);
    ...
    for(int i=0;i<Nbins;i++){
    	...
    	result[i]=log10(bin_integration_element*totalpixel);
...
//	double arrary[2][8]={{log10x,-6.7,-6.3,-5.533,-4.766,-4.0,-3.25,-1.9},{log10y,15.0620,14.4342,13.4563,12.1586,10.2764,8.55,6.314}};
	...
	if(flux>=0.0)dnds=pow(10,interp(log10(flux),length,(double *)arrary_pointer));
